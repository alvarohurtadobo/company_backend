...
@api_view(["GET"])
def use_and_empty(request, kit_id):
    kit = get_object_or_404(Kit, pk=kit_id)
    kit_serializer = KitSerializer(
        kit, many=False, data={"used_at_datetime": timezone.now(), "amount": 0}, partial=True)
    if kit_serializer.is_valid():
        kit_serializer.save()
...
@api_view(["GET"])
def transform_and_empty(request, kit_id):
    kit = get_object_or_404(Kit, pk=kit_id)
    kit_serializer = KitSerializer(
        kit, many=False, data={"transformed_at_datetime": timezone.now(), "amount": 0}, partial=True)
    if kit_serializer.is_valid():
        kit_serializer.save()
        return Response(status=status.HTTP_204_NO_CONTENT)
    else:
